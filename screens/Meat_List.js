...
function Meat_List({ navigation }) {
    const [meat, setMeat] = useState([]);
    const todoRef = firebase.firestore().collection('Meat');
    useEffect(() => {
        todoRef
            .onSnapshot(
                querySnapshot => {
    ...
    const searchFishes = (text) => {
        ...
        query.get().then((querySnapshot) => {
            ...
            setMeat(newFishes);
            setLastMeat(querySnapshot.docs[querySnapshot.docs.length - 1]);
        }).catch((error) => {
                                ...
                                <TouchableOpacity
                                    onPress={() => {
                                        if (item.status === "Còn món") {
                                            navigation.navigate('Detail_Meat', { foodID: item.id });
                                        }
                                    }}
                                    style={{
                                        marginLeft: 15,
