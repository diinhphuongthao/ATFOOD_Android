...
function Soup_List({ navigation }) {
    const [soup, setSoup] = useState([]);
    const todoRef = firebase.firestore().collection('Soup');
    useEffect(() => {
        todoRef
            .onSnapshot(
                querySnapshot => {
    ...
    const [lastSoup, setLastSoup] = useState(null);
    ...
    const searchFishes = (text) => {
        ...
        query.get().then((querySnapshot) => {
            ...
            setSoup(newFishes);
            setLastSoup(querySnapshot.docs[querySnapshot.docs.length - 1]);
        }).catch((error) => {
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <View style={{ justifyContent: 'center', paddingTop: 40, alignItems: 'center', marginRight: 15 }}>
                                <TouchableOpacity onPress={() => navigation.navigate('Detail_Soup', { foodID: item.id })} style={{ marginLeft: 15, justifyContent: 'center', borderWidth: 1, borderRadius: 10, marginLeft: 15, }}>
                                    <View style={{}}>
